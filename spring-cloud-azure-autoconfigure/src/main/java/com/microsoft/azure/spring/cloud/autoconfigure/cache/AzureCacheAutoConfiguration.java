...
public class AzureCacheAutoConfiguration {
    ...
    @Primary
    @Bean
    public RedisProperties redisProperties(Azure.Authenticated authenticated, AzureProperties azureProperties, AzureCacheProperties cacheProperties) throws IOException {
        String cacheName = cacheProperties.getName();

        RedisCache redisCache = authenticated.withDefaultSubscription().redisCaches().getByResourceGroup(azureProperties.getResourceGroup(), cacheName);
